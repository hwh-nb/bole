...
export default {
  ...
  methods: {
    zhaopin(){
      ...
      this.$data.sizeForm.or = "1"
      console.log("zhao",this.$data.sizeForm.or)
    },
    qiuzhi(){
      zhao.style = "display:block";
      this.$data.sizeForm.or = "2"
      console.log("qiu",this.$data.sizeForm.or)
    },
    userBlur(){
    ...
    onSubmit() {
      ...
      if(str == "true,true,true,true,true,true"){
        ...
        //获取表单数据
        this.$axios.post('http://localhost:3000/web/add',{user:this.$data.sizeForm.name, pwd:this.$data.sizeForm.pwd, phone:this.$data.sizeForm.phone, email:this.$data.sizeForm.email, or:this.$data.sizeForm.or}).then(res=>{
          console.log('res=>',res);
        })
